...
export function ThemeCustomizer({ open, onOpenChange }: ThemeCustomizerProps) {
  const { applyImportedTheme, isDarkMode, resetTheme, applyRadius, setBrandColorsValues, getPersistedThemeData, loadPersistedTheme } = useThemeManager()
  ...
  React.useEffect(() => {
    ...
    setSelectedTheme(persistedData.selectedTheme || "")
    setSelectedTweakcnTheme(persistedData.selectedTweakcnTheme || "")
    setSelectedRadius(persistedData.customRadius || "0.5rem")
  }, [getPersistedThemeData])

  // Re-apply persisted theme on theme mode changes
  React.useEffect(() => {
    loadPersistedTheme()
  }, [isDarkMode, loadPersistedTheme])
  ...
  const handleReset = () => {
    ...
    // 2. Completely remove all custom CSS variables and localStorage data
    resetTheme()
  ...
  const handleImport = (themeData: ImportedTheme) => {
    ...
    // Apply the imported theme (persistence handled in applyImportedTheme)
...
export function ThemeCustomizerTrigger({ onClick }: { onClick: () => void }) {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "fixed top-[95%] -translate-y-1/2 h-12 w-12 rounded-full shadow-lg z-50 bg-primary hover:bg-primary/90 text-primary-foreground cursor-pointer animate-spin",
